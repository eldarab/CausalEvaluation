...
def get_data(tokenizer):
    features = Features({
        ...
        'acceptability_sophiemarshall2': ClassLabel(num_classes=2, names=['unacceptable', 'acceptable'], id='tc_labels'),
        'is_books': ClassLabel(num_classes=2, names=['not_books', 'books'], id='cc_labels'),
        'sentiment': ClassLabel(num_classes=2, names=['negative', 'positive'], id='task_labels'),
    })
    dataset_f = load_dataset(
        'csv',
        data_files={'train': str(DATA_DIR / 'SAD_train_old.csv'),
                    'test': str(DATA_DIR / 'SAD_test_old.csv')},
        index_col=0,
        features=features
    )
    dataset_f = dataset_f.rename_column('acceptability_sophiemarshall2', 'tc_labels')
    dataset_f = dataset_f.rename_column('is_books', 'cc_labels')
    dataset_f = dataset_f.rename_column('sentiment', 'task_labels')
